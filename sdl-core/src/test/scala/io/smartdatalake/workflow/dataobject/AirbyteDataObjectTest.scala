...
class AirbyteDataObjectTest extends DataObjectTestSuite {
  ...
  test("wsl cmd test") {
    ...
    val dataObject = AirbyteDataObject("test", config, "mystream", cmd = CmdScript(winCmd = Some("wsl ./sdl-core/src/test/resources/airbyteConnectorMock.sh")))
    dataObject.prepare
    val actual = dataObject.getDataFrame()(session, contextExec)
