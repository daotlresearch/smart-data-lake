...
case class AirbyteDataObject(override val id: DataObjectId,
  ...
  private def launchConnector(op: Operation.Operation, config: Option[Config] = None, catalog: Option[ConfiguredAirbyteCatalog] = None, state: Option[String] = None)(implicit session: SparkSession, context: ActionPipelineContext): Iterator[AirbyteMessage] = {
