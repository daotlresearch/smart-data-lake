...
trait CmdScriptBase extends ParsableScriptDef with SmartDataLakeLogger {
  ...
  override def execStdOutString(configObjectId: ConfigObjectId, partitionValues: Seq[PartitionValues], parameters: Map[String,String], errors: mutable.Buffer[String] = mutable.Buffer())(implicit session: SparkSession, context: ActionPipelineContext): String = {
  ...
  override def execStdOutStream(configObjectId: ConfigObjectId, partitionValues: Seq[PartitionValues], parameters: Map[String, String], errors: mutable.Buffer[String] = mutable.Buffer())(implicit session: SparkSession, context: ActionPipelineContext): Stream[String] = {
    import sys.process._
    val cmd = getCmd(parameters)
    val errLogger = ProcessLogger.apply { err =>
      logger.error(err)
